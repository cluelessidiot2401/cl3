...
class Cipher:
	...
	def caesar(st,key,mode='encrypt'):
		st= st.lower()
		if mode not in ['encrypt','decrypt']: raise InvalidMode("Mode must be encrypt or decrypt")
    if key <0 or key > 25: raise InvalidKeyException("Key must be between 0 and 25")
